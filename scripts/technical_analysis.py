...
class TechnicalAnalysis:
    ...
    def calculate_dmi(self, period: int = 14) -> Tuple[pd.Series, pd.Series, pd.Series]:
        """计算DMI指标，返回+DI, -DI, ADX（修复DM互相依赖Bug，使用Wilder平滑）"""
        high = self.data['high'] # type: ignore
        low = self.data['low'] # type: ignore
        close = self.data['close'] # type: ignore

        # 修复：先计算原始 DM，基于原始值同时判断方向，避免互相依赖
        up_move = high.diff()
        down_move = -low.diff()
        raw_plus_dm = up_move.clip(lower=0)
        raw_minus_dm = down_move.clip(lower=0)
        condition = raw_plus_dm >= raw_minus_dm
        plus_dm = raw_plus_dm.where(condition, 0)
        minus_dm = raw_minus_dm.where(~condition, 0)

        tr1 = high - low
        tr2 = abs(high - close.shift())
        tr3 = abs(low - close.shift())
        tr = pd.concat([tr1, tr2, tr3], axis=1).max(axis=1)
        # 修复：ATR 同样使用 Wilder 平滑
    ...
    def calculate_rsi(self, period: int = 14) -> pd.Series:
        """计算RSI指标（Wilder平滑，与TradingView等主流平台一致）"""
        delta = self.data['close'].diff() # type: ignore
        gain = delta.where(delta > 0, 0)
        loss = -delta.where(delta < 0, 0)
        # 修复：使用 Wilder 平滑（alpha=1/period），而非简单滚动均值
    ...
    def calculate_atr(self, period: int = 14) -> pd.Series:
        """计算ATR（平均真实波幅，Wilder平滑，与TradingView等主流平台一致）"""
